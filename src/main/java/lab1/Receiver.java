...
public class Receiver {
    ...
    public static void decodePacket(final byte[] input_msg, Cryptor mc) throws BadPaddingException, IllegalBlockSizeException {
        if (input_msg[0] != 0x13) {
        ...
        final short crc1Evaluated = CRC16.crc16(input_msg, 0, 14);
        if (crc1Evaluated != crc1) {
            throw new IllegalArgumentException("CRC1 expected: " + crc1Evaluated + ", out was " + crc1);
        }
        decodeMessage(input_msg,mc);
    }
        public static void decodeMessage ( final byte[] input_msg, Cryptor mc) throws BadPaddingException, IllegalBlockSizeException {
            final int msgLenEnc = ByteBuffer.wrap(input_msg, 10, 4)
                    .order(ByteOrder.BIG_ENDIAN)
                    .getInt();
            ...
            byte[] jsonArray = new byte[msgLenDecr];
            System.arraycopy(decr_message, 0, jsonArray, 0, msgLenDecr);
            System.out.println("Your message: " + new String(jsonArray));
            final short crc2 = ByteBuffer.wrap(input_msg, 16 + msgLenEnc, 2)
                    .order(ByteOrder.BIG_ENDIAN)
                    .getShort();
            System.out.println("CRC 2: " + crc2);
            final short crc2Evaluated = CRC16.crc16(enc_message, 0, msgLenEnc);
